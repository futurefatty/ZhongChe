package com.neusoft.zcapplication.gestureLock.widget;
...
import com.neusoft.zcapplication.gestureLock.GesturePoint;
...
//		paint.setColor(Color.rgb(255, 200, 173));// 设置默认连线颜色
		paint.setColor(Color.argb(100,255, 200, 173));// 设置默认连线颜色
			...
			case MotionEvent.ACTION_UP:// 当手指抬起的时候
				if (isVerify) {
					...
					if (passWord.equals(passWordSb.toString())) {
						...
						//如果没有选择到图片，则不显示错误验证信息
						if(!"".equals(passWordSb.toString())){
							// 用户绘制的密码与传入的密码不同。
